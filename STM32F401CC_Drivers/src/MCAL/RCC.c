...
RCC_enuErrorStatus_t RCC_enuSelectPLLsSrc(uint32_t Copy_u32PLLSrc)
{
	...
	if ((Copy_u32PLLSrc == PLLSRC_HSI) || (Copy_u32PLLSrc == PLLSRC_HSE))
	{
		...
		else
		{
			...
			uint32_t LOC_u32LocalReg = RCC->PLLCFGR;
...
RCC_enuErrorStatus_t RCC_enuConfigMainPLL(uint32_t Copy_u32PLLM, uint32_t Copy_u32PLLN, uint32_t Copy_u32PLLP, uint32_t Copy_u32PLLQ)
{
	...
	else
	{
				...
				&& ((Copy_u32PLLQ >= PLLQ_2) && (Copy_u32PLLQ <= PLLQ_15)))
		{
			...
			uint32_t LOC_PLLSrc, LOC_VCOINValue, LOC_VCOOUTValue, LOC_PLLFreqValue, LOC_OTGFreqValue;
...
RCC_enuErrorStatus_t RCC_enuConfigPLLI2SN(uint32_t Copy_u32PLLM, uint32_t Copy_u32PLLI2SN, uint32_t Copy_u32PLLI2SR)
{
	...
	else
	{
				...
				&& ((Copy_u32PLLI2SR >= PLLI2SR_2) && (Copy_u32PLLI2SR <= PLLI2SR_7)))
		{
			...
			/* First get the PLL source to know the input clock frequency value that will used in
			 * the calculations and operations */
			uint32_t LOC_PLLSrc, LOC_VCOINValue, LOC_VCOOUTValue, LOC_I2SFreqValue;
			if ((RCC_enuGetPLLsSrc(&LOC_PLLSrc) == RCC_enuOk))
			{
				switch (LOC_PLLSrc)
				{
					...
					if((LOC_VCOINValue >= VCO_IN_FREQ_LWR_LMT) && (LOC_VCOINValue <= VCO_IN_FREQ_UPR_LMT))
					{
						...
						if ((LOC_VCOOUTValue >= VCO_OUT_FREQ_LWR_LMT) && (LOC_VCOOUTValue <= VCO_OUT_FREQ_UPR_LMT))
						{
							...
							if(LOC_I2SFreqValue <= PLLI2S_FREQ_UPR_LMT)
							{
								...
								/* Create a local variable as an image of the real register */
								uint32_t LOC_u32LocalPLLReg = RCC->PLLCFGR;
								uint32_t LOC_u32LocalPLLI2SReg = RCC->PLLI2SCFGR;
								/* Clean the bits before assigning in it */
...
RCC_enuErrorStatus_t RCC_enuSelectSysClk(uint32_t Copy_u32SysClk)
{
	...
	{
		/* Create a local variable as an image of the real register */
		uint32_t LOC_u32LocalReg = RCC->CFGR;
		/* Clean the bits before assigning in it */
...
RCC_enuErrorStatus_t RCC_enuConfigTimersPRES(uint32_t Copy_u32TIMsPRES)
{
	...
	{
		/* Create a local variable as an image of the real register */
		uint32_t LOC_u32LocalReg = RCC->DCKCFGR;
		/* Clean the bits before assigning in it */
